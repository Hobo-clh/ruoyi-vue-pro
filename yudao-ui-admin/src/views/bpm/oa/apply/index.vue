    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini"
                   v-hasPermi="['bpm:oa-apply:create']" @click="handleAdd">发起申请</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleCancel(scope.row)"
                     v-hasPermi="['bpm:oa-apply:create']" v-if="scope.row.result === 1">取消</el-button>
          <el-button size="mini" type="text" icon="el-icon-view" @click="handleDetail(scope.row)"
                     v-hasPermi="['bpm:oa-apply:query']">详情</el-button>
...
<script>
import { getApplyPage } from "@/api/bpm/apply"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // 执行查询
      getApplyPage(params).then(response => {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({ path: "/bpm/oa/apply/create"});
    },
    /** 详情按钮操作 */
    handleDetail(row) {
      this.$router.push({ path: "/bpm/oa/apply/detail", query: { id: row.id}});
    },
    /** 查看审批进度的操作 */
